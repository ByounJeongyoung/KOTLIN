...
이름이 동일한 것처럼 보이지만 매개변수가 서로 다른 형태를 취하거나
실행 결과를 다르게 가질 수 있는 것을 다형성(polymorphism) 이라고 합니다.
예를 들어 버튼의 ‘누르다’라는 행위에 대해 생각해 봅시다. 메서드로 설계하면 push() 정도가 되겠죠? 하지만 이 누르는 행위가
‘취소’ 버튼에 적용되면 프로그램 종료를 수행하고, ‘저장’ 버튼에 적용하면 작업을 저장하는 일을 진행합니다.
즉, ‘누르다’라는 같은 이름의 push() 메서드로 서로 다른 동작을 하는 것이죠.
이처럼 상위와 하위 클래스에서 메서드나 프로퍼티의 이름은 같지만 기존의 동작이 다른 동작으로 재정의하는 것을 오버라이딩(overriding)이라고 부릅니다.
...
class Parrot(
    ...
    beak: String,
    color: String,
    var language: String = "natural"            // language인자는 var로 선언되어 프로퍼티가 추가되었음을 알 수 있다.
...
fun main() {
    ...
    val parrot = Parrot(name = "myparrot", wing = 2, beak = "short", color = "multiple",language = "이것도 가능")
    // parrot.language = "English"

    println("Parrot: ${parrot.name}, ${parrot.wing}, ${parrot.beak}, ${parrot.color}, ${parrot.language}")
    parrot.sing(5) // 달라진 메서드 실행 가능
    parrot.fly()       // 상속의 이유! 함수를 설정하지 않아도 사용 가능하다
}
